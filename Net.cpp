...
void readFile(const char* filename, vector<Net>& Nets)
{
    ...
    while (getline(file, line))
    {
        if (line.find('{') != string::npos)
        {
            ...
            getline(file, line);
            sscanf_s(line.c_str(), " \"TX_COORD\":[%lf,%lf],", &net.TX.TX_COORD.first, &net.TX.TX_COORD.second);
            getline(file, line);
            pos = line.find('[');
...
int main()
{
    ...
    {
        cout << "ID: " << net.ID << endl;
        cout << "TX: " << (net.TX.TX_info.first == 0 ? "BLOCK" : "REGION") << "_" << net.TX.TX_info.second << endl;
        cout << "RX: [";
        for (size_t i = 0; i < net.RXs.size(); ++i)
        {
            cout << (net.RXs[i].RX_info.first == 0 ? "BLOCK" : "REGION") << "_" << net.RXs[i].RX_info.second << ", ";
        }
        cout << "\b\b]" << endl << setprecision(3) << setiosflags(ios::fixed);
        cout << "Num: " << net.num << endl;
        cout << "TX_COORD: [" << net.TX.TX_COORD.first << ", " << net.TX.TX_COORD.second << "]" << endl;
        cout << "RX_COORD: [";
        for (size_t i = 0; i < net.RXs.size(); ++i)
        {
            cout << "[" << net.RXs[i].RX_COORD.first << "," << net.RXs[i].RX_COORD.second << "], ";
        }
